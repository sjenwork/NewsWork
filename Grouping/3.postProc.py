...
class ref():
    def __init__(self):
        ...
        self.chem2casno, self.chem2ename  = self.__getName__()
...
class matchChem(header, ref):
    ...
    def __match__(self, df):
        df['MatchNo']       = df.ChemicalChnName.map(self.chem2matchno)
...
def status(df):
    df = df[~df.Chemical_material.isna()]
    df = df[df['pred_label']!='無關']
    df2 = pd.DataFrame()
    df2['id'] = df.id
    df2['status'] = '正常'
...
if __name__ == '__main__':
    ...
    print(f'   >>> 3.4 網路擷取分析_國內proc1_新聞對應化學物質') 
    ...
    c.push(df_proc  , '[ChemiBigData].[dbo].[網路擷取分析_國內proc1_新聞對應化學物質]')
